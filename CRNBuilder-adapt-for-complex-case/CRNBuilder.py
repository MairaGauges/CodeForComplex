...
from readVTK import initializeFromVTK, get_boundary_data_dim2
...
def get_massflow(y, z, vy, vz, rho, axial_n, y_lim, z_lim, Ny, Nz, boundary_data, radial_dir, axial_dir, inlets, dim2):
    ...
    for n in range(len(inlets)):
        inlet = inlets[n][0]
        boundary_data[inlet]['Ay'] = np.zeros(boundary_data[inlet]['U'].shape[0], dtype=float)
        boundary_data[inlet]['Az'] = np.zeros(boundary_data[inlet]['U'].shape[0], dtype=float)
        boundary_data[inlet]['my'] = np.zeros(boundary_data[inlet]['U'].shape[0], dtype=float)
        boundary_data[inlet]['mz'] = np.zeros(boundary_data[inlet]['U'].shape[0], dtype=float)
        lower_bound = inlets[n][1][1]
...
def main():
    ...
    if not dim2:
        ...
        filterArray = np.ones(shape=(Ny,Nz))
