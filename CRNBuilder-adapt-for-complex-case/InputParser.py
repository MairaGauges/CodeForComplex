...
def parse_input(filename):
    ...
    all_set = [False for _ in range(7)]
    ...
    for c in range(len(lines)):
        ...
        if command == "":
            ...
            if dim2: 
                if len(inlet_data) != 4:
                    raise RuntimeError("Cannot scrape inlet data, expected arguments number is 4")
                if inlet_data[0] == 'radial':
                    inlet_data[0] = 0
                elif inlet_data[0] == 'axial':
                    inlet_data[0] = 1
                else:
                    raise RuntimeError(f"{inlet_data[0]} is not a valid inlet argument. Valid arguments are 'radial' and 'axial'")
        ...
        elif command == 'InputType':
            if lines[c].split(':')[0] == 'type':
                if lines[c].split(':')[1] == '2d':
                    dim2 = True
                elif lines[c].split(':')[1] == 'wedge':
                    dim2 = False 
            all_set[6] = True     
    if prod(all_set) == 0:
    ...
    print(f"{inlets = }\n"
          ...
          f"{axial_dir = }\n"
          f"{T_threshold = }\n")
    return inlets, outlets, y_lim, z_lim, angle, pressure, radial_dir, axial_dir, T_threshold, dim2
